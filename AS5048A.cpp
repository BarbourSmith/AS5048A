 ...
 */
void AS5048A::AbsoluteAngleRotation (long *RotationAngle, long *AngleCurrent, long *AnglePrevious){

  if (*AngleCurrent != *AnglePrevious){

    //A transition from high to low
    if ((*AngleCurrent < 4096) && (*AnglePrevious > 12288)){
            *RotationAngle += abs(16384 - abs(*AngleCurrent - *AnglePrevious));
    }
    //A transition from low to high
    else if ((*AnglePrevious < 4096) && (*AngleCurrent > 12288)){
            *RotationAngle -= abs(16384 - abs(*AngleCurrent - *AnglePrevious));
    }
